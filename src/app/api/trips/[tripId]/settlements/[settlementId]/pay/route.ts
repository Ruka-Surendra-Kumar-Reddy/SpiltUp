...
/** POST /api/trips/[tripId]/settlements/[settlementId]/pay — payer marks paid. */
export async function POST(req: Request, { params }: { params: Promise<{ tripId: string; settlementId: string }> }) {
  try {
    ...
    if (s.fromId !== memberId) {
      return NextResponse.json({ error: "Only the payer can mark this as paid." }, { status: 403 });
    }
    if (s.status !== "pending") {
      return NextResponse.json({ error: "Settlement is not pending." }, { status: 400 });
    }
    await db.settlement.update({ where: { id: s.id }, data: { status: "paid" } });
